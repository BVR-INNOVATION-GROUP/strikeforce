...
const Option = (o: OptionI) => {
    return (
        <div
            ...
            <Activity mode={o?.icon ? "visible" : "hidden"}>
                {o?.icon}
            </Activity>
            <span> {o.label}</span>
        </div>
    ...
    return (
        <div>
            ...
            <div
                ...
                className='border relative border-custom  rounded-lg p-3 flex items-center justify-between'>
                <span>{props?.value ?
                    <div

                        className={`cursor-pointer rounded flex items-center gap-2 hover-bg-pale `}>
                        <Activity mode={props?.value?.icon ? "visible" : "hidden"}>
                            {props?.value?.icon}
                        </Activity>
                ...
                {/* dowpdown */}
                {
                    open && <motion.div
                        ...
                        className="bg-paper rounded-lg z-50 flex flex-col p-4 overflow-y-scroll min-w-[20vw] shadow absolute bottom-[110%] max-h-[40vh] shadow-custom w-full min-h-[20vh] left-0">
