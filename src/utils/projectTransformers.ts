 ...
 */
export const transformApplications = (applications: ApplicationI[], groups?: any[], users?: any[]) => {
    return applications.map(app => {
        ...
        } else if (app.applicantType === "INDIVIDUAL" && app.studentIds.length > 0 && users) {
        ...
        const members = memberIds.map(studentId => {
            ...
            return {
                ...
                avatar: user?.profile?.avatar || "https://images.pexels.com/photos/1239291/pexels-photo-1239291.jpeg"
            }
        })
 ...
 */
export const transformMilestones = (milestones: MilestoneI[], projectCurrency?: string) => {
    return milestones.map(milestone => ({
        ...
        progress: milestone.status === "IN_PROGRESS" ? 45 : milestone.status === "COMPLETED" ? 100 : undefined,
        completedDate: milestone.status === "COMPLETED" ? milestone.updatedAt : undefined
    }))
}
...
export const transformTeamMembers = (
    ...
    const roles = ["Lead Developer", "UI/UX Designer", "Backend Developer", "Frontend Developer", "Full Stack Developer", "QA Engineer"]
    ...
    const studentMembers = Array.from(allStudentIds)
        .map((studentId, idx) => {
            const user = users?.find(u => u.id === studentId)
    ...
    if (supervisorId && !studentIds.includes(supervisorId)) {
        ...
        if (supervisorUser) {
            ...
            teamMembers.push({
                ...
                avatar: supervisorUser.profile?.avatar || "https://images.pexels.com/photos/1239291/pexels-photo-1239291.jpeg",
                badges: {
                    isLeader: false,
                    isSupervisor: true,
