...
import { GraduationCap, Plus, Filter, CheckCircle2, XCircle, Eye, Edit, Trash2, Loader2 } from "lucide-react";
import { useRouter } from "next/navigation";
import Skeleton from "@/src/components/core/Skeleton";

/**
 * Universities Page - Stat cards, charts, table with filters
 */
export default function UniversitiesPage() {
    ...
    const fetchOrganizations = async () => {
        try {
            ...
            const universities = orgs.filter((org) => org.type === "UNIVERSITY");
            setOrganizations(universities);
        } catch (error) {
    ...
    const fetchProjects = async () => {
        try {
            ...
            result.projects.forEach((p) => {
                map.set(p.universityId, (map.get(p.universityId) ?? 0) + 1);
            });
    ...
    const handleDeapprove = async () => {
        ...
        try {
            ...
            showSuccess(`${org?.name || "University"} has been deapproved and set to pending review`);
        } catch (error) {
            console.error("Failed to deapprove organization:", error);
            showError("Failed to deapprove university. Please try again.");
        } finally {
            setApproving(null);
    ...
    const projectsPerUniversityData = useMemo(() => {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            showSuccess(`${org?.name || "University"} has been deleted`);
        } catch (error) {
    ...
    return (
        <div className="w-full flex flex-col min-h-full">
            ...
            <div className="flex-shrink-0 mb-8 flex items-center justify-between">
                <div>
                    <h1 className="text-[1rem] font-[600] mb-2">Universities</h1>
            ...
            {/* Stat Cards */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <StatCard icon={<GraduationCap size={20} />} title="Total Universities" value={stats.total} />
                <StatCard icon={<GraduationCap size={20} />} title="Pending" value={stats.pending} />
                <StatCard icon={<GraduationCap size={20} />} title="Approved" value={stats.approved} />
                <StatCard icon={<GraduationCap size={20} />} title="Rejected" value={stats.rejected} />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                <BarChart
                    ...
                    data={projectsPerUniversityData}
            ...
            <Card className="p-4">
                ...
                <DataTable
                    ...
                    onRowClick={(row) => {
                        router.push(`/super-admin/universities/${row.orgId}`);
                    }}
                />
            </Card>
                ...
                message={
                    <div className="space-y-2">
                        ...
                        <p className="text-sm opacity-75">
                            The university's KYC status will be changed from APPROVED to PENDING.
