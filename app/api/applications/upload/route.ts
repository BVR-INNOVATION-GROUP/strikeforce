/**
 * API Route: POST /api/applications/upload
 * Handles file uploads for application attachments (CVs, portfolios, etc.)
 */
 ...
 * Upload application attachment files
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate file count (max 5 files per application)
    if (files.length > 5) {
      return NextResponse.json(
        { error: "Maximum 5 files allowed per application" },
        { status: 400 }
      );
    }
    ...
    for (const file of files) {
      ...
      paths.push(`/uploads/applications/${uniqueFilename}`);
    }
